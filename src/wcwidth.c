...
int wcwidth_cjk(wchar_t ucs) {
  ...
  static const struct interval legacy_ja[] = {{0x00A2, 0x00A3},
                                              {0x00A5, 0x00A6},
                                              {0x00AC, 0x00AC},
                                              {0x00AF, 0x00AF},
                                              {0x2212, 0x2212}};
  ...
  if (bisearch(ucs, ambiguous, sizeof(ambiguous) / sizeof(struct interval) - 1))
    return 2;
#ifdef JA_LEGACY
  if (bisearch(ucs, legacy_ja, sizeof(legacy_ja) / sizeof(struct interval) - 1))
    return 2;
#endif /* JA_LEGACY */
  ...
  if (bisearch(ucs, emoji, sizeof(emoji) / sizeof(struct interval) - 1))
    return 2;
#endif /* EMOJI */
