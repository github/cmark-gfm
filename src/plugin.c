...
int cmark_plugin_register_syntax_extension(cmark_plugin *plugin,
  ...
  plugin->syntax_extensions = cmark_llist_append(
      &CMARK_DEFAULT_MEM_ALLOCATOR, plugin->syntax_extensions, extension);
  return 1;
}

cmark_plugin *cmark_plugin_new(void) {
  cmark_plugin *res = (cmark_plugin *)CMARK_DEFAULT_MEM_ALLOCATOR.calloc(
      1, sizeof(cmark_plugin));
...
void cmark_plugin_free(cmark_plugin *plugin) {
  cmark_llist_free_full(&CMARK_DEFAULT_MEM_ALLOCATOR, plugin->syntax_extensions,
